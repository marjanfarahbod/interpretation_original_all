...
def SDparse(file_path):
    ...
    with open(file_path, 'r') as f:
        ...
        for line in f:
            ...
            n = int(line_data[4])
            label = SDLabel(label_name,track_name,mean,sd,n)
...
class SDAnnotation:
    def __init__(self, file_path,*args):
        ...
        self.labels, self.label_names = SDparse(file_path)
        ...
        if len(args) > 0 and args[0] == "norm":
            ...
            raise Exception("ERROR in *args (input), nothing will happen")
...
with open(reference_anns_list, "r") as f:
    for line in f:
        ...
        if False: # XXX Skipping this check. These filenames are deprecated; if I do it again, improve the flow so that the names aren't duplicated.
...
def features_from_segtools_dir(summary_dirpath):
    ...
    for label, gencode_label_info in gencode.labels.items():
        ...
        for component_name, component_enrichments in gencode_label_info.component_enrichments().items():
            ...
            if component_name == "5' flanking: 10000 bp":
                ...
                # XXX skipping renaming of features; don't forget to replace it
    ...
    if not histone_path.exists():
        ...
        for label, histone_label_info in histone_signal.labels.items():
            ...
            print "------------------------"
            print vars(histone_label_info) # XXX
            print histone_signal.signal_distributions
            feature_name = histone
            #feature_name = feature_name_map[feature_name]
